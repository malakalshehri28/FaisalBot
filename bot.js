...
if (message.content.startsWith("مسح")) {
 let args = message.content.split(" ").slice(1)
    ...
    if (args > 1001) return message.reply("**يجب ان يكون عدد المسح أقل من 1000 .**").then(messages => messages.delete(5000000))
    if (!messagecount) return message.reply("**أختر كميه الرسائل المراد مسحها .**").then(messages => messages.delete(5000000))
    message.channel.fetchMessages({limit: messagecount + 1}).then(messages => message.channel.bulkDelete(messages));
    message.channel.send(`\`${args}\` : __عدد الرسائل التي تم مسحها __ `).then(messages => messages.delete(5000000));
  }
  });
